...
class Emitter():
    ...
    *    @param typ the type of the constant
    ...
    def emitArray(self, in_, idx,frame,sym_name):
        #in_: Type
        #frame: Frame
        #..., arrayref, index, value -> ...
        if type(in_.eleType) is IntType:
            typ = 'int'
        elif type(in_.eleType) is FloatType:
            typ = 'float'
        elif type(in_.eleType) is BoolType:
            typ = 'boolean'
        elif type(in_.eleType) is StringType:
            typ = 'java/lang/String'
        if type(idx) is cgen.Index:
            frame.pop()
            if type(in_.eleType) is StringType:
                return self.jvm.emitANEWARRAY(typ) + self.jvm.emitASTORE(idx.value)
            else:
                return self.jvm.emitNEWARRAY(typ) + self.jvm.emitASTORE(idx.value)
        elif type(idx) is cgen.CName:
            if type(in_.eleType) is StringType:
                return self.jvm.emitANEWARRAY(typ) + self.emitPUTSTATIC(idx.value+"."+sym_name, in_, frame)
            else:
                return self.jvm.emitNEWARRAY(typ) + self.emitPUTSTATIC(idx.value+"."+sym_name, in_, frame)
        else :
    ...
    *   @param in the type of the returned expression.
    ...
    def emitRETURN(self, in_, frame):
        ...
        elif type(in_) is cgen.ArrayPointerType or type(in_) is StringType or type(in_) is ArrayType:
